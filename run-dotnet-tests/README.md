...
Test .NET solutions or projects across multiple platforms with code coverage reporting. Sets up the .NET 9 (STS) and .NET 10 (LTS) SDKs side-by-side, restores from NuGet.org and GHCR (including private packages) without persisting credentials, runs tests with coverage collection, and uploads the report to **GitHub Code Quality** (native PR coverage).
