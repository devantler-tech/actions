...
#
# Input (stdin): the REST `pulls` list of CLOSED pull requests from the release
# branch, as returned by
#   gh api "repos/<repo>/pulls?state=closed&head=<owner>:<branch>&sort=created&direction=desc&per_page=100"
#
...
if ! jq -e 'type == "array"' >/dev/null 2>&1 <<<"$input"; then
  echo "::error::release PR lifecycle check: input is not a pull request list, so the check did not run"
  exit 2
fi
