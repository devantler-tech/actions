...
#
# Usage: compute-head-seen-floor.sh <repository> <pr-number> <head-sha> [own-run-id]
...
#                poison the floor and wedge arming; it is excluded.
#
...
#      committer date);
#   2. committer-date fallback when no other check suite exists yet (never an
#      empty, fail-open floor);
...
if [[ $# -lt 3 || $# -gt 4 ]]; then
  echo "usage: $0 <repository> <pr-number> <head-sha> [own-run-id]" >&2
  exit 2
fi
...
floor=$(gh api "repos/$repository/commits/$head_sha/check-suites" --paginate |
  jq -rs --arg own "$own_suite_id" '[.[].check_suites[]? | select((.id | tostring) != $own) | .created_at | select(. != null)] | min // empty')
if [[ -z "$floor" ]]; then
  floor=$(gh api "repos/$repository/commits/$head_sha" --jq .commit.committer.date)
fi
