...
while IFS= read -r fixture; do
  ...
  event_before="$(jq -r '.event_before' <<<"$fixture")"
  event_head="$(jq -r '.event_head' <<<"$fixture")"
  event_updated_at="$(jq -r '.event_updated_at' <<<"$fixture")"
  expected_exit="$(jq -r '.expected_exit' <<<"$fixture")"

  set +e
  jq -c '.timeline' <<<"$fixture" |
    bash "$script" "$event_action" "$event_updated_at" "$event_before" "$event_head"
  actual_exit=$?
  set -e
