...
# update); <out-dir>/<slug>.patch holds that skill's binary diff against HEAD, and stdout is a
# compact JSON array of {"slug","path"} sorted by path. A change that belongs to no skill fails the
# run rather than being dropped or bundled into another skill's pull request. The caller's own index
# is never touched: staging happens in a temporary copy.
...
git add -A -- "$dir" || fail "could not stage the update under $dir"
git diff --cached --name-only --no-renames -z -- "$dir" >"$work/changed" ||
  fail "could not list the changes under $dir"
...
while IFS= read -r skill; do
  ...
  git diff --cached --binary --no-renames -- "$skill" >"$out_dir/$slug.patch" ||
